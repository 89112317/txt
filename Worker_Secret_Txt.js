...
async function createDocument(request) {
  try {
    ...
    const id = generateId();
    const data = { text_doc };
    await Worker_Secret_txt.put(id, JSON.stringify(data));
...
async function getDocument(id) {
  try {
    const value = await Worker_Secret_txt.get(id);
    ...
    await Worker_Secret_txt.delete(id);
...
<body>
    ...
    <footer>      
      <p>© 秘密文档 - 极简、<a href="https://github.com/89112317/txt" target="_blank" rel="noopener noreferrer">开源</a>的在线文档。</p>
    </footer>
    <script>
...
function renderShareHTML(text_doc) {
  ...
  <body>
      <pre><span style="color: #0000ff;"><strong>3秒后显示内容</strong></span></pre>
  </body>
  </html>  
  `;
}
